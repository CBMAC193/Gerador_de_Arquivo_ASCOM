"""
Gerador de Diplomas (layout alinhado ao Agradecimento/Moeda)

Campos esperados em 'dados':
    - subtipo     : Tipo específico (ex.: Bravura, Dom Pedro II – Cavaleiro etc.)
...
def gerar_certificado_diploma(dados: dict) -> str:
    """
    Gera o Diploma com layout harmonizado ao Agradecimento.
    """
    ...
    imagem = find_model_image('Diplomas', subtipo) if subtipo else find_model_image('Diplomas')
    if imagem is None:
    ...
    faltando = [k for k, v in {
        "Subtipo": subtipo or "—", "Nome": nome, "Decreto": decreto,
        "Local": local, "Data": data_bruta, "Comandante": comandante
    }.items() if not v or v == "—"]
    if faltando:
    ...
    texto = (
        ...
        f"confere a <font name='Corsiva' color='red'>{nome}</font> o presente Diploma "
        f"<font name='Corsiva' color='red'>{subtipo}</font>, do Corpo de Bombeiros Militar do Estado do Acre, "
        "pelos relevantes serviços prestados à comunidade acreana."
    )
